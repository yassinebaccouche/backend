   ...
   const  encryptedPassword = await bcrypt.hash(password, 10);
    ...
    const user = await User.create({
      ...
      let token = await new Token({
        ...
        var message = `http://localhost:5002/user/verify/${user.id}`;
  ...
  console.log(email)   // Validate if user exist in our database
   ...
   if(user)
   {
    if (user.verified && (await bcrypt.compare(password, user.password))) {
      ...
      user.token=token;
      res.status(200).json({ success: true, message: "login avec succeés", user });
...
export async function LogidAD(req,res){
...
export async function forgetPass(req, res) {
  try {
    ...
    if (user) {
      // If the code is valid, return a success message with HTTP status 200
      res.status(200).json({ message: "Code is valid"});
    } else {
      // If the code is invalid, return an error message with HTTP status 400
      res.status(400).json({ message: "Invalid code" });
    }
  } catch (error) {
    // If an error occurs, log the error to the console and return an error message with HTTP status 500
    console.log(error);
    res.status(500).json({ message: "Internal server error" });
  }
  
  console.log("Forget password function executed");
}
...
export async function GetUser(req, res) {
  ...
  export async function GetallAdmin(req,res){
    ...
    try {
      ...
      var user = await User.find({role:"Admin"})
      if(user)
      {
        res.send(user)
        res.status(200).json(user)
